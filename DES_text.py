...
pbox_table = [15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25, 4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10, 3, 24]
    ...
    while (bv.more_to_read):
        ...
        if bitvec._getsize() > 0:
            if bitvec._getsize() < 64:
                numadd = 64 - bitvec._getsize()
                bitvec.pad_from_right(numadd)
            [LE, RE] = bitvec.divide_into_two()
                
            for i in range(16):
                newRE = RE.permute( expansion_permutation )
                out_xor = newRE.__xor__( round_key[i] )
                suboutput = substitute(out_xor)
                right_half = suboutput.permute(pbox_table)
                    
                final_RE = LE.__xor__(right_half)
                LE = RE
                RE = final_RE
                
            final_string = final_RE + LE
            final_stringlist.append(final_string.get_bitvector_in_hex())
                  
    return(final_stringlist)
...
def decrypt(message, keyfromfile):
    ...
    final_destringlist = []
    for j in range(0, len(bv) // 64):
        bitvec = bv[j * 64:j * 64 + 64]
        if bitvec._getsize() > 0: 
            if bitvec._getsize() < 64:
                numadd = 64 - bitvec._getsize()
                bitvec.pad_from_right(numadd)
            ...
            for i in range(16):
                ...
                final_RE = LE ^ (right_half)
                LE = RE
                RE = final_RE

            final_string = final_RE + LE
            final_destringlist.append(final_string.get_bitvector_in_ascii())

    return(final_destringlist)
